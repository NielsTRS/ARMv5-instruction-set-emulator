...
int memory_read_word(memory mem, uint32_t address, uint32_t *value, uint8_t be) {
    if(address < mem->size - 3 && mem != NULL && mem->data != NULL){
        *value = (uint16_t)mem->data[address] | (uint16_t)mem->data[address + 1] << 8 | (uint16_t)mem->data[address + 2] << 16 | (uint16_t)mem->data[address + 3] << 24;
        return 0;
    }
    return -1;
}
...
int memory_write_byte(memory mem, uint32_t address, uint8_t value) {
    if(address < mem->size && mem != NULL && mem->data != NULL){
...
int memory_write_half(memory mem, uint32_t address, uint16_t value, uint8_t be) {
    if(address < mem->size - 1 && mem != NULL && mem->data != NULL){
